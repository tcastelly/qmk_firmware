...
typedef struct PACKED {
    ...
    uint16_t bottoming_calibration_reading; // bottoming reading
} eeprom_ec_config_t;
...
typedef struct {
    ...
    uint16_t rescaled_apc_actuation_threshold;    // threshold for key press in mode 0 rescaled to actual scale
    uint16_t rescaled_apc_release_threshold;      // threshold for key release in mode 0 rescaled to actual scale
    uint16_t rescaled_rt_initial_deadzone_offset; // threshold for key press in mode 1 (initial deadzone) rescaled to actual scale
    uint8_t  rescaled_rt_actuation_offset;        // offset for key press in mode 1 rescaled to actual scale
    uint8_t  rescaled_rt_release_offset;          // offset for key release in mode 1 rescaled to actual scale
    uint16_t extremum;                            // extremum values for mode 1
    uint16_t noise_floor;                         // noise floor detected during startup
    bool     bottoming_calibration;               // calibration mode for bottoming out values (true: calibration mode, false: normal mode)
    bool     bottoming_calibration_starter;       // calibration mode for bottoming out values (true: calibration mode, false: normal mode)
    uint16_t bottoming_calibration_reading;       // bottoming reading
} ec_config_t;
